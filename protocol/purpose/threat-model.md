...
- **Integrity envelope construction:** exactly as defined in `../core/canonicalization.md#integrity-envelope`.
- **Commitment-layer Merkle hashing rules:** domain separation + ordering rules as defined in `../applications/economics/settlement-bindings.md`.
...
Changing a fixed invariant requires a protocol MAJOR version bump (see `../../governance/versioning.md`).

## Integrity Envelope

**Normative source:** `../core/canonicalization.md#integrity-envelope` is authoritative. This section is a security-focused restatement; in case of conflict, `../core/canonicalization.md` wins.
...
|---|---|---|---|
| Spend-stream signing keys | Authenticity/integrity of spend-stream events for a given `protocolVersion` | Ground truth of receipts; user ownership; fraud intent | `../core/spend-event.md` + deployment configuration (v1), evolvable via `protocolVersion` |
| System-stream authority keys (Authority Registry) | Authenticity/integrity of system-stream events (commitments, authority changes) for a `chainId` | Correctness of off-chain reward policy; spend truth beyond what is committed | `../core/spend-event.md`, `../applications/economics/settlement-bindings.md` |
...
| Token issuer authorization | That a token signature key is an authorized issuer key for the referenced `issuedBy` | That the token’s claim corresponds to external reality beyond the claim definition | `../portability/spend-attestation-token.md` + the applicable trust root mapping (Authority Registry or configured issuer set) |
| Merchant-claim verifier authorization (optional extension) | That a merchant claim verifier issued a signed claim attestation for a bounded store identity scope | Spend truth; payment settlement; merchant intent for any spend; correctness of private evidence beyond the attestation semantics | `../extensions/merchant-authority.md` + deployment authorization mapping |
| Proof-validator finality certificates | That a quorum of selected, registered proof validators independently recomputed a deterministic public statement from committed material and co-signed the identical result | Ground truth of receipts; honesty of the verification service beyond its committed output; payout authority; production chain finality | `../core/admission.md` + authority-signed registry/assignment artifacts |

Proof-validator registry and quorum consumers MUST enforce
[`validator-signing-key-independence.md`](../core/validator-signing-key-independence.md).
...
- **Authority rotation (system-stream):** system-stream events MUST be verified against the Authority Registry validity window at the event-effective time (see `../core/spend-event.md` and `../applications/economics/settlement-bindings.md`).
...
1. **Schema presence:** require all required envelope fields for the stream type (`../core/spend-event.md`); reject if missing.
2. **Protocol version:** require `protocolVersion`; reject if missing or unsupported.
3. **Integrity:** recompute `eventHash` per `../core/canonicalization.md#integrity-envelope`; reject on mismatch.
...
- `FRAUD_FLAGGED` is observational and MUST NOT participate in attestation state transitions (`../core/spend-event.md`).
