...
| `MERCHANT_PRODUCT_CATEGORY_RELATIONSHIP` | The first profile is the sealed alpha's single-product relation: exact product, brand, category, inclusive Condition-relative time, minimum quantity, minimum amount and currency over one approved purchase witness. The separately adopted Protocol candidate `campaign.atomicProductSetPurchase.solanaGroth16.v1` proposes set membership for product, brand and category under the same one-witness shape. | The exact-product profile is internally adopted with executed alpha lineage; runtime unavailable. The product-set profile has merged Protocol and Platform contract ports, but no circuit artifact, setup, key, proof, program or Solana acceptance exists. | `CampaignEpochV2.conversionRuleRef` and proof `ruleCommitment` equal `conditionId`; the absolute evaluated rule is separately bound. Exact product/status sources, V2 dependencies and projection are required. `BUYER_STATE_MERCHANT_MATCH_V1` remains a separate statement profile. The candidate has no machine proof-profile content reference, so it cannot yet be selected by the existing `CONVERSION` compiler/package or substituted for the exact-product profile. |
| `FREQUENCY_INTENSITY` | The registered internal non-production `campaign.distinctPurchaseCount.audience.groth16.v1` profile covers only `BUYER_STATE_DISTINCT_PURCHASE_COUNT_GTE_V1` with threshold four, the inclusive `asOf - 44 days` through `asOf` window, `CAMPAIGN_INFLUENCED`, one namespace/issuer and positive-lower-bound-only meaning. It does not cover `BUYER_STATE_SPEND_TOTAL_CENTS_GTE_V1`. | Exact circuit, setup receipt and verifying key are registered for non-production offline conformance; runtime unavailable. The separate Family B V2 verifier-source pin at `crinkl-platform@b394513c` has 14 historical-B host regressions and an SBF ELF, but no current-profile proof, Devnet program or Devnet `ACCEPT`; it does not upgrade this row. | Context V2 and the complete A2c graph are required. A non-production offline-conformance Campaign Epoch may select the exact profile ref, but runtime selection requires a separate verifier registry binding; no program, runtime or Solana acceptance exists. |
