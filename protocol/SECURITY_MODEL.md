...
- **Integrity envelope construction:** exactly as defined in `DATA_STRUCTURES.md#integrity-envelope`.
- **Commitment-layer Merkle hashing rules:** domain separation + ordering rules as defined in `COMMITMENT_LAYER.md`.
...
Changing a fixed invariant requires a protocol MAJOR version bump (see `PROTOCOL_EVOLUTION.md`).

## Integrity Envelope

**Normative source:** `DATA_STRUCTURES.md#integrity-envelope` is authoritative. This section is a security-focused restatement; in case of conflict, `DATA_STRUCTURES.md` wins.
...
|---|---|---|---|
| Spend-stream signing keys | Authenticity/integrity of spend-stream events for a given `protocolVersion` | Ground truth of receipts; user ownership; fraud intent | `EVENTS.md` + deployment configuration (v1), evolvable via `protocolVersion` |
| System-stream authority keys (Authority Registry) | Authenticity/integrity of system-stream events (commitments, authority changes) for a `chainId` | Correctness of off-chain reward policy; spend truth beyond what is committed | `EVENTS.md`, `COMMITMENT_LAYER.md` |
...
| Token issuer authorization | That a token signature key is an authorized issuer key for the referenced `issuedBy` | That the token’s claim corresponds to external reality beyond the claim definition | `TOKENS.md` + the applicable trust root mapping (Authority Registry or configured issuer set) |
...
- **Authority rotation (system-stream):** system-stream events MUST be verified against the Authority Registry validity window at the event-effective time (see `EVENTS.md` and `COMMITMENT_LAYER.md`).
...
1. **Schema presence:** require all required envelope fields for the stream type (`EVENTS.md`); reject if missing.
2. **Protocol version:** require `protocolVersion`; reject if missing or unsupported.
3. **Integrity:** recompute `eventHash` per `DATA_STRUCTURES.md#integrity-envelope`; reject on mismatch.
...
Tokens are verification-ready bundles built from the same primitives: signed events and (optionally) committed Merkle roots. Token verification MUST reduce to the event and commitment verification rules defined in this document and in COMMITMENT_LAYER.md (see TOKENS.md).
...
- `FRAUD_FLAGGED` is observational and MUST NOT participate in attestation state transitions (`EVENTS.md`).
