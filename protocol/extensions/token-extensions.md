...
Future extensions SHOULD NOT introduce new token categories that duplicate or reinterpret protocol truth, reward eligibility, or verification semantics. Campaign rule composition SHOULD use the canonical `CampaignEpoch` and `ProofOfMatch` vocabulary (`../applications/campaigns/README.md`, `../applications/conditions/proof-of-match.md`) over existing token and proof surfaces.
