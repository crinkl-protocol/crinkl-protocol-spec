...
|-----------|----------|----------------------|
| `verificationVersion` | ../core/ingestion.md | Rule evolution without breaking old verifications |
| `SPEND_CORRECTED` | ../core/spend-event.md | Labeled error examples for model training |
| `StoreEntryV1.aliases` | store-registry.md | Taxonomy expansion from OCR mismatches |
| `softExtractedFields` | ../core/ingestion.md | Soft vs hard comparison for calibration |
| `riskFlags` | ../core/canonicalization.md | Predictive signal for deferral/review |
| Merkle batching | ../applications/economics/settlement-bindings.md | Cost amortization at scale |
| Append-only event stream | ../core/verification-state.md | Replayable labeled dataset |
...
- **../core/ingestion.md** — Defines two-tier verification, `verificationVersion`, normalization rules
- **../core/spend-event.md** — Defines `SPEND_CORRECTED`, `correctedFields`, append-only semantics
- **store-registry.md** — Defines alias expansion, `StoreEntryV1`, registry versioning
- **../core/canonicalization.md** — Defines `riskFlags`, field schemas
- **../applications/economics/settlement-bindings.md** — Defines Merkle batching, `leafCount`
- **../core/verification-state.md** — Defines append-only event stream, replay semantics
