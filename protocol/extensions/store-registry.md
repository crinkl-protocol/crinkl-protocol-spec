...
>
> This document defines a **portable, publicly replicable** way to standardize store and store-location identifiers across implementations without changing the core token set in `../portability/spend-attestation-token.md`.
...
- This registry does **not** define a new protocol trust root. It reuses existing issuer/authority key authorization rules (see `../purpose/threat-model.md#trust-roots`).
...
`storeId` is an **Identifier string** (see `../core/canonicalization.md#identifier`) representing a merchant family identity (typically chain/brand-level).

**Normative recommendation (namespacing):** `storeId` SHOULD be namespaced to avoid collisions and to allow multiple authorities to interoperate. Because core `Spend.storeId` is a normalized identifier (see `../core/canonicalization.md#spend`), prefer **prefix-style namespacing** using the same character set (e.g., lowercase + digits + hyphens).
...
**Store hash (core, already in `../portability/spend-attestation-token.md`):**
...
- `tokenHash` and signature rules match `../portability/spend-attestation-token.md` (`tokenHash = sha256(RFC8785(unsignedToken))`).
- `entriesRoot` MUST be computed using the Merkle rules in `../applications/economics/settlement-bindings.md#merkle-tree`:
...
- `categories` MUST be sorted lexicographically (UTF-16 code units) before hashing (see RFC 8785 array rules note in `../core/canonicalization.md`).
...
StoreLocationEntryV1 {
  ...
  cbsaCode?: CBSACode,              // e.g. "12420" (OMB CBSA metro area) — see ../core/canonicalization.md
...
This extension reuses the Merkle proof structure from `../applications/economics/settlement-bindings.md` (siblings are hashes; direction bits unnecessary due to sorted-pair hashing).
...
- Unknown `schemaVersion` MUST be rejected for portable verification of this extension artifact (see `../../governance/versioning.md`).
- Changes that alter Merkle hashing rules, leaf field semantics, or hash preimages require a new `schemaVersion`.

## Example (non-normative)

- Example store list: `examples/store-registry/v1/stores.json`
- Example location list: `examples/store-registry/v1/locations.json`
