...
Already defined in `../portability/spend-attestation-token.md`:
...
For v1, statement types are a closed set captured in `zk-circuit-catalog.md`. The foundation introduces two statement families that are sufficient to support the demo promo flow end-to-end.
...
Foundation statement example B (normative intent; full mapping in `zk-circuit-catalog.md`):
...
See `../portability/spend-attestation-token.md#optional-zk-statement-proof-normative` for the portable proof bundle shape and verification steps.
