...
|-----------|----------|----------------------|
| `verificationVersion` | VERIFICATION_PIPELINE.md | Rule evolution without breaking old verifications |
| `SPEND_CORRECTED` | EVENTS.md | Labeled error examples for model training |
| `StoreEntryV1.aliases` | STORE_REGISTRY.md | Taxonomy expansion from OCR mismatches |
| `softExtractedFields` | VERIFICATION_PIPELINE.md | Soft vs hard comparison for calibration |
| `riskFlags` | DATA_STRUCTURES.md | Predictive signal for deferral/review |
| Merkle batching | COMMITMENT_LAYER.md | Cost amortization at scale |
| Append-only event stream | STATE_MACHINES.md | Replayable labeled dataset |
...
- **VERIFICATION_PIPELINE.md** — Defines two-tier verification, `verificationVersion`, normalization rules
- **EVENTS.md** — Defines `SPEND_CORRECTED`, `correctedFields`, append-only semantics
- **STORE_REGISTRY.md** — Defines alias expansion, `StoreEntryV1`, registry versioning
- **DATA_STRUCTURES.md** — Defines `riskFlags`, field schemas
- **COMMITMENT_LAYER.md** — Defines Merkle batching, `leafCount`
- **STATE_MACHINES.md** — Defines append-only event stream, replay semantics
