...
- `tokenHash` is computed from the canonical JSON of an unsigned token object (see `TOKENS.md`).
...
|---|---|---|---|---|
| Event integrity | `eventHash` | RFC 8785 canonical JSON | Structural (envelope includes `eventName`, stream key, `protocolVersion`) | `DATA_STRUCTURES.md#integrity-envelope` + `EVENTS.md` |
| Token signature digest | `tokenHash` | RFC 8785 canonical JSON | Structural (unsigned token includes `tokenType`, `schemaVersion`) | `TOKENS.md` |
| Store hash (token field) | `storeHash` | bytes | ASCII prefix `"crinkl.store.v1:"` + canonical `storeId` | `TOKENS.md#spend-attestation-token` |
| Commitment-layer Merkle leaf | (binary hash) | bytes | Prefix `0x00` | `COMMITMENT_LAYER.md#merkle-tree` |
| Commitment-layer Merkle internal | (binary hash) | bytes | Prefix `0x01` | `COMMITMENT_LAYER.md#merkle-tree` |
| Blinded recipient id | `recipientId` | bytes | ASCII prefix `"crinkl.recipient.v1:"` + context | `COMMITMENT_LAYER.md#recipient-blinding` |
| ZK statement identity | `statementId` | RFC 8785 canonical JSON | Structural (statement object is type-tagged) | `ZK_FOUNDATION.md`, `ZK_LAYER.md` |
...
- **Range:** non-negative arbitrary precision integer; implementations MUST support at least unsigned 64-bit range
- **Rationale:** GMV aggregates can exceed JS safe integer range; string encoding ensures cross-language consistency (same pattern as Points/Satoshis)
...
Every protocol **event** MUST include the following envelope fields. Ledger entries (AttestationLedgerEntry, RewardLedgerEntry) are derived views projected from events and do not carry their own envelope.
...
`prevHash` is defined per stream key: spend-stream events chain per `spendId`, and system-stream events chain per `chainId` (see EVENTS.md). The first event in any stream MUST set `prevHash` to `null`.
...
To prevent cross-stream ambiguity and replay, verifiers MUST validate that the event object conforms to the correct envelope schema for its stream type (see `EVENTS.md`).
...
Payload for the `RECEIPT_UPLOADED` event (see EVENTS.md). This structure is embedded inside a SpendStreamEvent envelope.
...
8. Example: `"Wal★Mart Supercenter #1234"` → `"walmart-supercenter-1234"`
...
FSM state for spend-stream (see STATE_MACHINES.md).
...
| `geo_mismatch` | Store location inconsistent with wallet history |

### EventName

Event type identifier (see EVENTS.md for full list).
...
- **Type:** JSON object
- **Constraints:** structure depends on EventName; see EVENTS.md for payload schemas
...
Token bundles (see TOKENS.md) are composed of existing protocol objects (events, leaves, proofs). When a token bundle includes protocol events or leaves, verifiers MUST treat them as the canonical inputs to hashing/signature verification:
