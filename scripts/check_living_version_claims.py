...
def validate_registry(registry: dict[str, Any], release_manifest: dict[str, Any]) -> None:
    ...
    require(release_manifest.get("releaseVersion") == "1.0.0-rc.7", "current source candidate version drift")
    require(release_manifest.get("status") == "RELEASE_CANDIDATE_NOT_PUBLISHED", "current source candidate status drift")
    require(release_manifest.get("requiredTag") == "v1.0.0-rc.7", "current source candidate tag drift")
...
def validate_documents(documents: dict[str, str]) -> None:
    ...
    required = {
        ...
        "08-governance/versioning.md": ("`v1.0.0-rc.4` is the latest released public package. Current public repository source candidate: **1.0.0-rc.7** (`RELEASE_CANDIDATE_NOT_PUBLISHED`), conformance suite 4; it is unreviewed, unpublished, not publishable, and does not inherit rc.5 review.", "Historical exact reviewed source candidate: **1.0.0-rc.5** (`REVIEWED_CANDIDATE_NOT_PUBLISHED`)", "`v1.0.0-rc.3` and `v1.0.0-rc.4` are released public packages; rc.4 is the\nlatest released package."),
