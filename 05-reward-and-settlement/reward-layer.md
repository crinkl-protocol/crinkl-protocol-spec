...
**Issuance binding (normative):** reward issuance events are spend-stream events. `REWARD_*_ISSUED.prevHash` MUST equal the then-current spend-stream head `eventHash` for the `spendId` (see `../01-core/spend-event.md#cross-fsm-ordering-constraints-normative`).
