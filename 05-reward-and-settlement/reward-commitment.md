...
1. Verify `systemEvents` as a contiguous, fork-free system-stream for `chainId` (integrity envelope + `prevHash` chaining) and verify authority validity per `../01-core/spend-event.md` and `settlement-bindings.md#authority-registry`.
