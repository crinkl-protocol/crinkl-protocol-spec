...
Validator membership, selection, and quorum are governed by authority-signed registry and assignment artifacts (`ValidatorRegistrySnapshotV1`, `ValidatorAssignmentV1`), as consumed by the finality trust root in `../05-reward-and-settlement/density-burn.md`.
...
- The Verification Service key set and the validator set MUST be disjoint, so the division of power cannot collapse to one key (root separation, `../05-reward-and-settlement/density-burn.md`).
...
4. Every attestation whose canonical head is included in the finalized statement's committed leaf set is admitted, with inclusion provable by Merkle path against the committed root (`../03-portability/verifier-requirements.md`).
...
- Record-level value movement — reserve depletion and burn, Verified GMV consumed as a protocol figure, campaign settlement against verified outcomes — MUST consume validator-finalized statements, never an operator counter (`../05-reward-and-settlement/density-burn.md`).
...
- `../05-reward-and-settlement/density-burn.md` — the finality trust root and joint-root consumption rules.
- `../03-portability/verifier-requirements.md` — Merkle inclusion procedure for coverage proofs.
