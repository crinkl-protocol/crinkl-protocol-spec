...
2. Verify `prevHash` chaining per `../01-core/spend-event.md#ordering-rules`; reject forks, and treat missing history as `IncompleteHistory` for verification purposes.
...
- **System-stream events** MUST be signed by a registered authority for the `chainId` at the event-effective time (Authority Registry; see `../05-reward-and-settlement/settlement-bindings.md`).
...
**Event-level idempotency (normative):** Idempotency is defined by `eventId` (see `../01-core/spend-event.md#eventid-derivation-normative`).
