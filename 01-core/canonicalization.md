...
- `tokenHash` is computed from the canonical JSON of an unsigned token object (see `../03-portability/spend-attestation-token.md`).
...
| Token signature digest | `tokenHash` | RFC 8785 canonical JSON | Structural (unsigned token includes `tokenType`, `schemaVersion`) | `../03-portability/spend-attestation-token.md` |
| Store hash (token field) | `storeHash` | bytes | ASCII prefix `"crinkl.store.v1:"` + canonical `storeId` | `../03-portability/spend-attestation-token.md#spend-attestation-token` |
| Commitment-layer Merkle leaf | (binary hash) | bytes | Prefix `0x00` | `../05-reward-and-settlement/settlement-bindings.md#merkle-tree` |
| Commitment-layer Merkle internal | (binary hash) | bytes | Prefix `0x01` | `../05-reward-and-settlement/settlement-bindings.md#merkle-tree` |
| Blinded recipient id | `recipientId` | bytes | ASCII prefix `"crinkl.recipient.v1:"` + context | `../05-reward-and-settlement/settlement-bindings.md#recipient-blinding` |
...
Reward ledger entries are defined in `../05-reward-and-settlement/reward-layer.md`. They reuse the scalar encodings in this file, but reward issuance is downstream of Core spend attestation.
...
Token bundles (see ../03-portability/spend-attestation-token.md) are composed of existing protocol objects (events, leaves, proofs). When a token bundle includes protocol events or leaves, verifiers MUST treat them as the canonical inputs to hashing/signature verification:
