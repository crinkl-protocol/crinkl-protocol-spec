...
Reward ledger events are downstream of Core spend attestation. They are listed here because they share spend-stream ordering and `prevHash` replay rules; their reward semantics are defined in `../05-reward-and-settlement/reward-layer.md`.
...
Commitment-layer event semantics are defined in `../05-reward-and-settlement/settlement-bindings.md` and `../04-condition-layer/campaign-commitment.md`. They are listed here to keep the protocol event catalog replayable.
...
`REWARD_BATCH_COMMITTED.payload.schemaVersion` identifies the commitment leaf schema (see ../05-reward-and-settlement/settlement-bindings.md). Schema v2 enables compact spend↔reward linkage proofs.
...
- Verifiers SHOULD apply chain-specific finality thresholds before treating those events as stable; finality rules are part of chain bindings (see `../05-reward-and-settlement/settlement-bindings.md#chain-bindings`).
